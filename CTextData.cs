...
namespace InterfaceLocalizer.Classes
{
    ...
    public class CTextData
    {
        ...
        public CTextData(string _phrase, string _eng, string _filename, Stack<string> _tags, string _sourcePhrase = "", bool _translated = false, bool _active = true)
        {
            phrase = _phrase;
            filename = _filename;
        ...
        public void addDialogsToManager(string filename)
        {
            ...
            foreach (XElement item in doc.Root.Elements())
            {
                ...
                if (!dialogsDict.ContainsKey(DialogID))
                    dialogsDict.Add(DialogID, new CTextData(title, old.title, filename, copy, source.title, translated, Active));
                else
                    dialogsDict.Add(adder2++, new CTextData(title, old.title, filename, copy, source.title, translated, Active));
                if (!dialogsDict.ContainsKey(DialogID+adder))
                    dialogsDict.Add(DialogID + adder, new CTextData(text, old.text, filename, copy, source.text, translated, Active));
                else
                    dialogsDict.Add(adder2++, new CTextData(text, old.text, filename, copy, source.text, translated, Active));
                tags.Pop();
            }
        }
